...
def test_get_movie_item_found(client, db_session):
    db_session.query(Movie).delete()
    db_session.commit()
...
def test_get_movie_item_not_found(client, db_session):
    db_session.query(Movie).delete()
    db_session.commit()
...
def test_post_movie_creates_new(client, db_session):
    db_session.query(Movie).delete()
    db_session.commit()
...
def test_put_movie_updates(client, db_session):
    db_session.query(Movie).delete()
    db_session.commit()
    ...
    response = client.put("/movies/7", json=payload)
    assert response.status_code == 200
    data = response.json()
