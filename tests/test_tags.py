...
def test_get_tag_item_found(client, db_session):
    db_session.query(Tag).delete()
    db_session.commit()
...
def test_get_tag_item_not_found(client, db_session):
    db_session.query(Tag).delete()
    db_session.commit()
...
def test_post_tag_creates_new(client, db_session):
    db_session.query(Tag).delete()
    db_session.commit()
...
def test_put_tag_updates(client, db_session):
    db_session.query(Tag).delete()
    db_session.commit()
...
def test_delete_tag_removes(client, db_session):
    db_session.query(Tag).delete()
    db_session.commit()
    ...
    response_get = client.get(f"/tags/{tag_id}")
